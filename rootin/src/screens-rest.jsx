    ...
    <div style={{ padding: '32px 40px', maxWidth: 640 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 20, marginBottom: 32 }}>
        <div style={{
          ...
          justifyContent: 'center', fontWeight: 700, fontSize: 24,
        }}>
          {nickname.charAt(0)}
        </div>
        <div>
          <div style={{ fontSize: 18, fontWeight: 600, color: 'var(--ink)' }}>{nickname}</div>
          <div style={{ fontSize: 12, color: 'var(--ink-3)', fontFamily: 'var(--font-mono)' }}>@{USER.handle}</div>
        </div>
      ...
      {editing ? (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <div>
            <div className="eyebrow" style={{ marginBottom: 6 }}>닉네임</div>
            <input
              value={nickname}
              onChange={e => setNickname(e.target.value)}
              style={{
                width: '100%', padding: '10px 12px', borderRadius: 8,
                border: '0.5px solid var(--rule)', fontSize: 13.5,
                fontFamily: 'var(--font-body)', boxSizing: 'border-box',
              }}
            />
          </div>
          <div>
            <div className="eyebrow" style={{ marginBottom: 6 }}>한 줄 소개</div>
            <textarea
              value={bio}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          <div style={{ padding: 16, background: 'var(--paper-2)', borderRadius: 10, fontSize: 13.5, color: 'var(--ink-2)', lineHeight: 1.7 }}>
            {bio}
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12 }}>
            {[
              { label: '총 TIL', value: USER.totalTil },
              { label: '현재 연속', value: `${USER.streak}일` },
              { label: '최고 연속', value: `${USER.bestStreak}일` },
            ].map(item => (
              <div key={item.label} style={{
                padding: '16px', background: '#fff', borderRadius: 10,
                border: '0.5px solid var(--rule)', textAlign: 'center',
              }}>
                <div style={{ fontSize: 22, fontWeight: 700, color: 'var(--moss)', fontFamily: 'var(--font-display)' }}>{item.value}</div>
                <div style={{ fontSize: 11, color: 'var(--ink-3)', marginTop: 4 }}>{item.label}</div>
              </div>
            ))}
          </div>
          <div style={{ fontSize: 11.5, color: 'var(--ink-3)' }}>
            가입일 {USER.joinedAt} · {USER.email}
          </div>
        </div>
      )}
    </div>
...
export function AuthScreen({ onAuth }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      setError(err.body?.message ?? '요청 중 오류가 발생했습니다.');
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleGoogle() {
    setError('');
    // Google One Tap / OAuth 연동 시 idToken을 받아 아래에 전달
    // 현재는 플레이스홀더
    alert('구글 로그인 연동 준비 중입니다.');
  }

  const inputStyle = {
    width: '100%', padding: '11px 14px', borderRadius: 9,
    border: '0.5px solid var(--rule)', fontSize: 14,
    fontFamily: 'var(--font-body)', boxSizing: 'border-box',
    outline: 'none',
  };
  ...
  return (
    <div style={{
      ...
      <div style={{
        width: 400, background: '#fff', borderRadius: 18,
        border: '0.5px solid var(--rule)',
        padding: '40px 36px',
        ...
        <div style={{ textAlign: 'center', marginBottom: 28 }}>
          <div style={{ fontFamily: 'var(--font-display)', fontSize: 28, fontWeight: 700, color: 'var(--ink)', letterSpacing: '-0.02em' }}>Rootin</div>
          <div style={{ fontSize: 12, color: 'var(--moss)', marginTop: 4 }}>루틴처럼, 뿌리처럼</div>
        </div>

        {/* 탭 */}
        <div style={{ display: 'flex', background: 'var(--paper-2)', borderRadius: 10, padding: 3, marginBottom: 24 }}>
          {['login', 'signup'].map(t => (
            <button
              key={t}
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          {error && (
            <div style={{ fontSize: 12.5, color: '#e05252', padding: '8px 12px', background: '#fff5f5', borderRadius: 7 }}>
              {error}
            </div>
          )}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, margin: '20px 0' }}>
          ...
          <span style={{ fontSize: 11.5, color: 'var(--ink-3)' }}>또는</span>
          <div style={{ flex: 1, height: '0.5px', background: 'var(--rule)' }} />
        </div>
...
export { CollectionScreen, AIScreen, ProfileScreen };
