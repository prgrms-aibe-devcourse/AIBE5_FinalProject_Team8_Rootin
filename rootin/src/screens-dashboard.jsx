  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 6 }}>
        {/* weekday labels */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: gap, fontFamily: 'var(--font-display)', fontSize: 9, color: 'var(--ink-4)', marginTop: 1 }}>
          {weekDays.map((d, i) => (
            ...
            <div key={wi} style={{ display: 'flex', flexDirection: 'column', gap }}>
              {week.map((v, di) => (
                <div key={di} title={`${wi}주 ${di}일 — ${v}글`} style={{
                  ...
                  background: colors[v],
                  border: v === 0 ? '0.5px solid var(--line-1)' : 'none',
                }} />
              ))}
            </div>
          ))}
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 14, fontFamily: 'var(--font-display)', fontSize: 10.5, color: 'var(--ink-3)' }}>
        <span>적음</span>
        {colors.map((c, i) => (
          <div key={i} style={{ width: 11, height: 11, borderRadius: 2.5, background: c, border: i === 0 ? '0.5px solid var(--line-1)' : 'none' }} />
        ))}
        <span>많음</span>
        <span style={{ marginLeft: 'auto', color: 'var(--ink-2)' }}>글자수로 농도 표현 · 총 91일 중 <b style={{ color: 'var(--navy-700)' }}>47일 기록</b></span>
      </div>
    </div>
  );
}
    ...
    <div style={{ display: 'flex', alignItems: 'flex-end', gap: 4, height: 60 }}>
      {days.map((d, i) => (
        <div key={i} style={{
          ...
          background: d.active ? 'linear-gradient(180deg, #3d8b5e, #2e6b48)' : 'var(--line-1)',
          borderRadius: 3,
        ...
        <div key={w.day} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6, height: '100%' }}>
          <div style={{ flex: 1, width: '100%', display: 'flex', alignItems: 'flex-end' }}>
            <div style={{
              ...
              minHeight: 4,
              background: w.count > 0 ? 'var(--green-500)' : 'var(--line-1)',
              borderRadius: '6px 6px 2px 2px',
              position: 'relative',
            }}>
              {w.count > 0 && <div style={{ position: 'absolute', top: -18, left: 0, right: 0, textAlign: 'center', fontFamily: 'var(--font-display)', fontSize: 11, fontWeight: 600, color: 'var(--navy-700)' }}>{w.count}</div>}
            </div>
          </div>
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 22 }}>
      <svg width="140" height="140" viewBox="0 0 140 140">
        <circle cx="70" cy="70" r={R} fill="none" stroke="var(--line-1)" strokeWidth="14" />
        {segs.map((s, i) => {
          const colors = ['var(--navy-700)', 'var(--green-500)', 'var(--brown-400)', 'var(--green-200)'];
        ...
        <text x="70" y="68" textAnchor="middle" style={{ fontFamily: 'var(--font-display)', fontSize: 20, fontWeight: 700, fill: 'var(--navy-700)' }}>{total}</text>
        ...
        {segs.map((s, i) => {
          ...
          return (
            <div key={s.id} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12 }}>
              ...
              <span style={{ color: 'var(--ink-1)' }}>{s.emoji} {s.name}</span>
...
function InterestFlow() {
  ...
  const colors = { coding: 'var(--navy-700)', english: 'var(--green-500)', reading: 'var(--brown-400)', workout: 'var(--green-200)' };
  return (
    <div>
...
function GoalRow({ goal }) {
  return (
    <div style={{
      ...
      padding: '12px 14px',
      borderRadius: 10,
      background: goal.done ? 'var(--green-50)' : 'var(--paper)',
      border: '0.5px solid var(--line-1)',
    }}>
      <div style={{
        width: 22, height: 22, borderRadius: 7,
        background: goal.done ? 'var(--green-500)' : '#fff',
        border: goal.done ? 'none' : '1px solid var(--line-2)',
        color: '#fff',
        display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0,
      }}>
        {goal.done && Icon.check}
      </div>
      <div style={{ flex: 1, fontSize: 13.5, color: 'var(--ink-1)', textDecoration: goal.done ? 'line-through' : 'none', opacity: goal.done ? 0.6 : 1 }}>{goal.label}</div>
      <div style={{ fontFamily: 'var(--font-display)', fontSize: 11.5, fontWeight: 600, color: goal.done ? 'var(--green-700)' : 'var(--ink-3)' }}>+{goal.point}P</div>
    </div>
  );
}
    ...
    <div style={{ padding: 32, display: 'flex', flexDirection: 'column', gap: 20, maxWidth: 1280, margin: '0 auto' }}>
      ...
      <Card padding={24} style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 22 }}>
          ...
          <div style={{ flex: 1 }}>
            <div className="eyebrow" style={{ color: 'var(--green-700)' }}>오늘의 한 줄</div>
            <div style={{ fontFamily: 'var(--font-display)', fontSize: 22, fontWeight: 600, color: 'var(--navy-700)', marginTop: 6, letterSpacing: '-0.01em' }}>
              "매일의 기록이 뿌리가 되어 꽃을 피웁니다"
            </div>
            <div style={{ fontSize: 12.5, color: 'var(--ink-2)', marginTop: 6 }}>
              연속 <b style={{ color: 'var(--green-700)' }}>{USER.streak}일</b> 기록 중 — 어제까지 6,420자, 오늘은 아직 비어있어요.
            </div>
          </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.6fr 1fr', gap: 16 }}>
            ...
            <div style={{ display: 'flex', gap: 4 }}>
              {['3개월', '6개월', '1년'].map((t, i) => (
                <button key={t} style={{
                  ...
                  border: i === 0 ? 'none' : '0.5px solid var(--line-2)',
        ...
        <Card padding={22}>
          <SectionHeader eyebrow="오늘 · 2026.05.22" title="오늘의 목표" action={
            <span style={{ fontFamily: 'var(--font-display)', fontSize: 11.5, color: 'var(--green-700)', fontWeight: 600 }}>
              80 / 110P
            </span>
          } />
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }}>
        <Card padding={22}>
          ...
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, marginBottom: 12 }}>
            <span style={{ fontFamily: 'var(--font-display)', fontSize: 40, fontWeight: 700, color: 'var(--green-700)', letterSpacing: '-0.03em' }}>{USER.streak}</span>
      ...
      <Card padding={22}>
        ...
        <InterestFlow />
        <div style={{ marginTop: 12, padding: '12px 14px', borderRadius: 10, background: 'var(--bg-soft)', fontSize: 12.5, color: 'var(--ink-2)', borderLeft: '2px solid var(--green-500)' }}>
